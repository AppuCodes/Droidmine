...
public class NetHandlerLoginClient implements INetHandlerLoginClient
{
    ...
    public NetHandlerLoginClient(NetworkManager p_i45059_1_, ClientEngine mcIn, GuiScreen p_i45059_3_)
    {
    ...
    public void handleEncryptionRequest(S01PacketEncryptionRequest packetIn)
    {
        ...
        if (this.mc.getCurrentServerData() != null && this.mc.getCurrentServerData().func_181041_d())
        {
            try
            {
                this.getSessionService().joinServer(this.mc.getSession().profile(), this.mc.getSession().token, s1);
            }
