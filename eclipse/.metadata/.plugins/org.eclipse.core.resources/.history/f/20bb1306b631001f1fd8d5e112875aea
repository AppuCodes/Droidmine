...
public class Minecraft implements IThreadListener
{
    ...
    private void setNativeBarWindows(long handle)
    {
        ...
        memory.setInt(0, 1);
        WinDef.HWND handleWindow = new WinDef.HWND(Pointer.createConstant(handle));
        DwmApi.INSTANCE.DwmSetWindowAttribute(handleWindow, DwmApi.DWMWA_USE_IMMERSIVE_DARK_MODE, new WinDef.LPVOID(memory), new WinDef.DWORD(WinDef.DWORD.SIZE));
        memory.close();
    }
    ...
    private void sendClickBlockToController(boolean leftClick)
    {
        ...
        if (this.leftClickCounter <= 0 && leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
        {
    ...
    protected long getHandle()
    {
        try
        {
            Method method = Display.class.getDeclaredMethod("getImplementation");
            method.setAccessible(true);
            Object object = method.invoke(null);
            Class clazz = Class.forName("org.lwjgl.opengl.WindowsDisplay");
            if (!clazz.isInstance(object)) throw new Exception("The current platform must be Windows!");
            method = clazz.getDeclaredMethod("getHwnd");
            method.setAccessible(true);
    ...
    public interface DwmApi extends StdCallLibrary
    {
        ...
        WinNT.HRESULT DwmSetWindowAttribute(WinDef.HWND hwnd, WinDef.DWORD dwAttribute, WinDef.LPVOID pvAttribute, WinDef.DWORD cbAttribute);
    }
}