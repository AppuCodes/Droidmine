...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround));
                }
                else if (flag2)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.onGround));
                }
                else if (flag3)
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer.C05PacketPlayerLook(this.rotationYaw, this.rotationPitch, this.onGround));
                ...
                {
                    this.sendQueue.sendPacket(new C03PacketPlayer(this.onGround));
                }
            }
            else
            {
                this.sendQueue.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, this.rotationYaw, this.rotationPitch, this.onGround));
                flag2 = false;
            }
    ...
    public void onLivingUpdate()
    {
        ...
        {
            if (this.sprintToggleTimer <= 0 && !this.mc.options.keyBindSprint.isKeyDown())
            {
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && this.mc.options.keyBindSprint.isKeyDown())
        {
            this.setSprinting(true);
        }
    ...
    public void openInventory()
    {
        ...
        {
            this.sendQueue.sendPacket(new C16PacketClientStatus(C16PacketClientStatus.EnumState.OPEN_INVENTORY_ACHIEVEMENT));
