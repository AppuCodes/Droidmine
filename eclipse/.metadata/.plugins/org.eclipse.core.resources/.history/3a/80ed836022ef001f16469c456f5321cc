...
public class GuiIngame extends Gui
{
    ...
    public GuiIngame(ClientEngine mcIn)
    {
        this.mc = mcIn;
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        int j1 = arraylist1.size() * (this.getFontRenderer().FONT_HEIGHT + 1);
        int k1 = p_180475_2_.getScaledHeight() / 2 + j1 / 2;
        byte b0 = 3;
        ...
        for (Object score1 : arraylist1)
        {
            ...
            int l = k1 - k * (this.getFontRenderer().FONT_HEIGHT + 1);
            int i1 = p_180475_2_.getScaledWidth();
            drawRect(j - 2, l - 1, i1, l + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            ...
            if (k == arraylist1.size())
            {
                ...
                drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 4, i1, l - 1, 1610612736);
                drawRect(j - 2, l, i1, l, 1342177280);
                this.getFontRenderer().drawString(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - this.getFontRenderer().FONT_HEIGHT - 2, 553648127);
            }
        }
    }
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            {
                this.lastSystemTime = ClientEngine.getSystemTime();
            ...
            if (ClientEngine.getSystemTime() - this.lastSystemTime > 1000L)
            {
                this.playerHealth = i;
    ...
    {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0 && !BossStatus.bossName.isEmpty())
        {
