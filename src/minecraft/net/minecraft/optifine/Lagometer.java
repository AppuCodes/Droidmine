...
{
    private static ClientEngine mc;
    ...
    public static void showLagometer(ScaledResolution p_showLagometer_0_)
    {
        if (options != null)
        {
            if (options.ofLagometer || options.field_181657_aC)
            {
                ...
                GlStateManager.get().ortho(0.0D, (double)mc.displayWidth, (double)mc.displayHeight, 0.0D, 1000.0D, 3000.0D);
                ...
                f1 = Config.get().limit(f1, 0.0F, 1.0F);
