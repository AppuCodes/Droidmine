...
public class GuiAchievements extends GuiScreen implements IProgressMeter
{
    ...
    {
        if (keyCode == this.mc.options.keyBindInventory.getKeyCode())
        {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        if (this.loadingAchievements)
        {
            ...
            this.drawCenteredString(this.fontRendererObj, lanSearchStates[(int)(ClientEngine.getSystemTime() / 150L % (long)lanSearchStates.length)], this.width / 2, this.height / 2 + this.fontRendererObj.FONT_HEIGHT * 2, 16777215);
        }
        else
        {
    ...
    protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_)
    {
        ...
        int j1 = l + 17;
        this.zLevel = 0.0F;
        GlStateManager.get().depthFunc(518);
        GlStateManager.get().pushMatrix();
        GlStateManager.get().translate((float)i1, (float)j1, -200.0F);
        GlStateManager.get().scale(1.0F / this.field_146570_r, 1.0F / this.field_146570_r, 0.0F);
        ...
        for (int l3 = 0; (float)l3 * f - (float)j2 < 155.0F; ++l3)
        {
            ...
            GlStateManager.get().color(f2, f2, f2, 1.0F);

            for (int i4 = 0; (float)i4 * f1 - (float)i2 < 224.0F; ++i4)
            {
                random.setSeed((long)(this.mc.getSession().playerID.hashCode() + k1 + i4 + (l1 + l3) * 16));
        ...
        for (int i6 = 0; i6 < AchievementList.achievementList.size(); ++i6)
        {
            ...
            if (l6 >= -24 && j7 >= -24 && (float)l6 <= 224.0F * this.field_146570_r && (float)j7 <= 155.0F * this.field_146570_r)
            {
                ...
                {
                    float f5 = 0.75F;
                    GlStateManager.get().color(f5, f5, f5, 1.0F);
                }
                else if (this.statFileWriter.canUnlockAchievement(achievement2))
                {
                    float f6 = 1.0F;
                    GlStateManager.get().color(f6, f6, f6, 1.0F);
                }
                else if (l7 < 3)
                {
                    float f7 = 0.3F;
                    GlStateManager.get().color(f7, f7, f7, 1.0F);
                }
                else if (l7 == 3)
                {
                    float f8 = 0.2F;
                    GlStateManager.get().color(f8, f8, f8, 1.0F);
                }
                else
                {
                    if (l7 != 4)
                    ...
                    float f9 = 0.1F;
                    GlStateManager.get().color(f9, f9, f9, 1.0F);
                }
                ...
                {
                    float f10 = 0.1F;
                    GlStateManager.get().color(f10, f10, f10, 1.0F);
