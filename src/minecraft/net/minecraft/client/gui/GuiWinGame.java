...
public class GuiWinGame extends GuiScreen
{
    ...
    {
        this.mc.player.sendQueue.sendPacket(new C16PacketClientStatus(C16PacketClientStatus.EnumState.PERFORM_RESPAWN));
        ...
        if (this.field_146582_i == null)
        {
            ...
            try
            {
                ...
                while ((s = bufferedreader.readLine()) != null)
                {
                    ...
                    for (s = s.replaceAll("PLAYERNAME", this.mc.getSession().name); s.contains(s1); s = s2 + EnumChatFormatting.WHITE + EnumChatFormatting.OBFUSCATED + "XXXXXXXX".substring(0, random.nextInt(4) + 3) + s3)
                    {
                        int j = s.indexOf(s1);
                        s2 = s.substring(0, j);
                ...
                while ((s = bufferedreader.readLine()) != null)
                {
                    s = s.replaceAll("PLAYERNAME", this.mc.getSession().name);
                    s = s.replaceAll("\t", "    ");
                    this.field_146582_i.addAll(this.mc.fontRendererObj.listFormattedStringToWidth(s, i));
                    this.field_146582_i.add("");
                }
